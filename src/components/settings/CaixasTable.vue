<template>
  <Card>
    <CardContent class="p-0">
      ...
      <div v-else-if="caixas.length === 0" class="flex flex-col items-center justify-center p-8">
        <Inbox class="h-12 w-12 text-muted-foreground mb-4" />
        <p class="text-sm font-medium mb-1">Nenhuma caixa de entrada encontrada</p>
        <p class="text-sm text-muted-foreground mb-4">
          Comece criando sua primeira caixa de entrada
        </p>
              ...
              <TableCell class="text-right">
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    <DropdownMenuItem @click="handleEdit(caixa)">
                      <Edit class="mr-2 h-4 w-4" />
                      Editar
                    </DropdownMenuItem>
                    <DropdownMenuItem @click="handleDelete(caixa)" class="text-destructive">
...
import { Plus, Inbox, Loader2, MoreVertical, Edit, Trash2 } from 'lucide-vue-next';
...
// Sorting
const sortable = useSortable<CaixaEntrada>(() => props.caixas);
...
const paginatedCaixas = computed(() => {
...
const emit = defineEmits<{
  edit: [caixa: CaixaEntrada];
  delete: [caixa: CaixaEntrada];
  create: [];
}>();
