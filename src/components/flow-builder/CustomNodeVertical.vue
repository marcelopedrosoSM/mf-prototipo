<template>
  <!-- Input Handle - start não tem entrada -->
  <Handle
    ...
    <div class="flex flex-col h-full">
      ...
      <template v-if="!isEntryExit">
        ...
        <!-- Metadados (variável, opções, condições) -->
          ...
          <!-- Vertical Mode Legend: List outputs inside the card -->
          <div v-if="hasMultipleOutputs" class="mt-2 space-y-1">
            ...
            >
              <span 
                class="font-bold shrink-0 w-3 h-3 flex items-center justify-center rounded-full bg-muted text-muted-foreground"
              >
                {{ index + 1 }}
              </span>
  ...
  <!-- Multi-Output Handles (Sibling of Card - On Top) -->
  <div 
    v-if="hasMultipleOutputs" 
    class="absolute bottom-4 left-0 w-full flex justify-center gap-4 z-50 pointer-events-none"
  >
    <div 
      v-for="(output, index) in visibleOutputs" 
      :key="index"
    ...
    >
      <Handle
        ...
        class="!w-4 !h-4 !border-4 !border-background pointer-events-auto transition-all hover:!w-5 hover:!h-5 !absolute top-1/2 left-1/2 -translate-x-1/2 translate-y-1/2"
        :style="{ backgroundColor: handleColor }"
      />
    </div>
  </div>
...
const borderColor = computed(() => {
  ...
  if (isExecuting.value) return 'hsl(var(--success))';
  if (wasExecuted.value) return 'hsl(var(--success))';
